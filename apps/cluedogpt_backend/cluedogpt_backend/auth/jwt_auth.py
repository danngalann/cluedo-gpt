...
def create_access_token(data: UserJwt, expires_delta: timedelta | None = None) -> tuple[str, int]:
    ...
    data.type = "access"
    data.iat = int(datetime.now(UTC).timestamp())
    data.exp = int(expire.timestamp())
    data.iss = "cluedogpt"

    encoded_jwt = jwt.encode(data.model_dump(), settings().jwt_secret_key, algorithm=settings().jwt_algorithm)
    return encoded_jwt, data.exp


def create_refresh_token(data: UserJwt, expires_delta: timedelta | None = None) -> tuple[str, int]:
